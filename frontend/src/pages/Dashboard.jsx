...
import { Upload, Search, Wand2, Loader2, Camera, Edit2, Check, Crown, ExternalLink, TrendingDown, Package, Star, Truck, ShoppingBag, IndianRupee, Sparkles } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.detail || 'Image analysis failed. Please try another image.'
  ...
  const handleGetDeals = async () => {
    ...
    const productToSearch = isEditingProduct ? editedProduct : step1Data.predicted_item
    ...
    setLoadingStep(2)
    try {
      const data = await getDeals(productToSearch, step1Data.search_id, pincode)
  ...
  const getSortedProducts = () => {
    ...
    if (comparisonData.amazon) {
      products.push({ site: 'Amazon', data: comparisonData.amazon, logo: '/amazon_logo.jpeg' })
    }
    if (comparisonData.flipkart) {
      products.push({ site: 'Flipkart', data: comparisonData.flipkart, logo: '/flipkart_logo.jpeg' })
    }
    if (comparisonData.snapdeal) {
      products.push({ site: 'Snapdeal', data: comparisonData.snapdeal, logo: '/snapdeal_logo.png' })
    }
    
    return products.sort((a, b) => (a.data.price || Infinity) - (b.data.price || Infinity))
  }
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 bg-amber-100 px-4 py-2 rounded-full mb-4 border border-amber-200">
            ...
            <span className="text-amber-900 font-light tracking-wide text-sm">AI-POWERED PRICE COMPARISON</span>
          </div>
          <h1 className="font-serif text-4xl md:text-5xl text-amber-900 mb-3">Find Your Best Deal</h1>
          <p className="text-amber-700 font-light text-lg">Upload, identify, and compare prices in seconds</p>
        </div>
            ...
            <label className="relative aspect-video md:aspect-square border-2 border-dashed border-amber-300 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-amber-400 hover:bg-amber-50/50 transition group overflow-hidden">
              {preview ? (
                ...
                <div className="text-center p-6">
                  ...
                  <span className="text-amber-900 font-medium block mb-1">Upload Product Image</span>
                  <span className="text-sm text-amber-600 font-light">PNG, JPG, WEBP up to 10MB</span>
                </div>
              )}
              <input 
                type="file" 
            ...
            <div className="flex flex-col justify-center space-y-4">
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-4">
                <div className="flex items-start gap-3">
                  <Package className="text-amber-600 mt-1 flex-shrink-0" size={24} />
                  <div>
                    <h3 className="font-medium text-amber-900 mb-1 text-lg">Quick Tips</h3>
                    <p className="text-sm text-amber-700 font-light">Ensure good lighting and the product is clearly visible in the frame</p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex-1">
                  {isEditingProduct ? (
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex-1">
                        <p className="text-xs text-amber-700 font-medium mb-1 uppercase tracking-wide">Product Identified</p>
                        <p className="text-xl md:text-2xl font-serif text-amber-900">{editedProduct}</p>
                      </div>
                      <button
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
              ...
              {getSavings() > 0 && (
                <div className="flex items-center gap-2 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-300 px-5 py-3 rounded-full shadow-md">
                  ...
                  <span className="text-green-700 font-semibold text-lg">Save up to ₹{getSavings().toLocaleString('en-IN')}</span>
                </div>
              )}
            </div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {(data.review_count || data.reviews) && (
                                <span className="text-xs text-stone-500">
                                  ({data.review_count?.toLocaleString('en-IN') || data.reviews} reviews)
                                </span>
                              )}
                            </div>
                          )}
                        ...
                        {data.delivery_date ? (
                          <div className="space-y-2">
                            <div className="flex items-center justify-center gap-2 text-stone-700">
                              ...
                              <span className="text-sm font-medium">{data.delivery_date}</span>
                            </div>
                          ...
                          <span className="text-stone-400 text-sm">Check on site</span>
                        )}
                      </div>
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-300 rounded-2xl p-5 shadow-md">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-sm font-semibold text-amber-900">Lowest Price</span>
                </div>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-300 rounded-2xl p-5 shadow-md">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-sm font-semibold text-blue-900">Platforms</span>
                </div>
              ...
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-300 rounded-2xl p-5 shadow-md">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-sm font-semibold text-green-900">Savings</span>
                </div>
        ...
        {comparisonData && getSortedProducts().length === 0 && (
          <div className="bg-white/80 backdrop-blur-sm border border-amber-200/60 rounded-3xl p-12 text-center shadow-lg">
            ...
            <p className="text-2xl text-amber-900 font-serif mb-2">No products found</p>
            <p className="text-amber-700 font-light mb-6">Try editing the product name or upload a different image</p>
            <button
              onClick={() => setIsEditingProduct(true)}
